#!/bin/bash
#############################################################################
# 功能: 测试 draw_table.sh 中的核心 gawk 绘制表格功能
# 命令：echo -e '1\n2\n3' | sh gawk.sh
#############################################################################

gawk -F '\t' -v 'table_s=+++++++++,---|||' -v color_s=-4,-8,-4 'BEGIN{
    }{
        # ------------------------------------------ 遍历每行记录全局变量 ------------------------------------------
        ...
        if (NF > 1) {
            ...
            for (i = 1; i <= length(cols_len); i++) {
                max_line_len = max_line_len + cols_len[i] + 2 # 每列需要包含2个分隔符, 防止内容和制表符紧挨着
            }
        ...
        } else if (diff_length < 0) { # 如果单列最大总长度小于行的最大总长度, 那么单列长度要和最大行总长度保持一致
